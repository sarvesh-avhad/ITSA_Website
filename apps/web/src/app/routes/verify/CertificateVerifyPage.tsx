...
import { Loader2, CheckCircle, XCircle, ShieldCheck, Download, Calendar, User, Trophy, CalendarDays, ExternalLink, Activity, AlertCircle } from 'lucide-react';
        ...
        <div className="bg-white/[0.02] border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl backdrop-blur-xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center text-center mb-10">
            ...
            <h1 className="text-3xl font-bold mb-3">
              {isValid ? 'Certificate Verified' : cert && cert.status === 'REVOKED' ? 'Certificate Revoked' : 'Verification Failed'}
            </h1>
            <p className="text-muted-foreground text-lg max-w-md">
              {isValid 
                ? 'This is a valid, officially issued certificate.'
                : cert && cert.status === 'REVOKED' 
                ? 'This certificate has been officially revoked by the issuer.'
                : 'The certificate number or verification token is invalid or does not exist.'}
            </p>
          </div>

          {(isValid || (cert && cert.status === 'REVOKED')) && (
            <div className="space-y-6">
              ...
              <div className="bg-black/40 rounded-2xl p-6 border border-white/5 shadow-inner">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <div className="text-sm text-muted-foreground mb-1 flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4" /> Issued By
                    </div>
                    <div className="font-semibold text-lg text-white">ITSA</div>
                    <div className="text-sm text-muted-foreground">Dr. D. Y. Patil Institute of Technology</div>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1 font-mono">Certificate Number</div>
